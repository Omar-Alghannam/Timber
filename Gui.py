...
voice_btn = tk.Button(side_frame, text="Voice ON", bg="#00FFA3", fg="#1C1446", font=("Segoe UI", 11, "bold"),
                      relief=tk.FLAT, command=toggle_voice)
...
def listen_in_background():
    ...
    if not voice_enabled or voice_listening:
        root.after(10000, listen_in_background)
        return
    ...
    def listen():
        ...
        with mic as source:
            ...
            try:
                ...
                if text.strip():
                    insert_message(text, sender="user")
                    response = get_response(text)
                    if isinstance(response, list):
                        insert_message("", sender="bot", clickable_links=response)
                        speak_async("Here are some links for you.")
                    else:
                        insert_message(response, sender="bot")
                        speak_async(response)
